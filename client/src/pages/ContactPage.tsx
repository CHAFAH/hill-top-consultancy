...
function ContactHeader({ onMenu }: { onMenu: () => void }) {
  return <header className="inner-header contact-header"><a className="wordmark hilltop-wordmark" href="/" aria-label="Hill-Top Consultancy home"><LogoLockup variant="dark" /></a><nav className="desktop-nav inner-nav" aria-label="Primary navigation"><ServicesMegaMenu /><SiteMegaMenu label="About us" href="/about" intro="About Hill-Top" title="A partner built for lasting value." items={aboutItems} /><SiteMegaMenu label="Industries" href="/industries" intro="Industries we serve" title="Deep context. Better outcomes." items={industryItems} /><a href="/success-stories">Success stories</a><a href="/insights">Insights <ChevronDown size={15} /></a></nav><div className="inner-header-actions"><GlobalSearch /><a className="contact-button inner-contact" href="/contact">Contact us</a><button className="icon-button inner-menu-button" onClick={onMenu} aria-label="Open navigation"><Menu size={24} /></button></div></header>;
}
...
export default function ContactPage() {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setSubmitError(error instanceof Error ? error.message : "Unable to send inquiry. Please email contact@hilltopconsultancy.com.");
